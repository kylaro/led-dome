#include "SnakeFirework.h"
#include "HEXPulse.h"
#include "Shimmer.h"
#include "../Interface/XBOX.h"
void SnakeFirework::init() {
...
void SnakeFirework::run() {
	...
	for (int k = 0; k < trail.size(); k++) {
		...
		trail[k].hsv.v *= 0.92;
		//ihsv_f ihsv = trail[k];
		LEDs->forceHSV(trail[k].i, trail[k].hsv);
	}
	//light up curnode
	pos += velocity;
	velocity *= 0.97;
	...
	while (pos > 1) {
		...
		if (velocity < 1.2 && curNode->nodeImpact()) {
			...
			snake->init();
			if (xbox::getLeftThumbActive(snake->player))
			{
				snake->curNode = curNode->getNeighborOrThisAngle(xbox::getLeftThumbAngle(snake->player));
			}
			else {
				snake->curNode = curNode->getNeighborOrThis();
			}
			engine->queueApply(snake);
