...
void Pershing::run(bool real) {
	...
	if (snake1_alive && snake2_alive){
		...
		//snake 2 collision code:
		if (snake2->curNode->nodeImpact() && snake2->prevNode->nodeImpact()) {
			xyz_f snake2_node_xyz = snake2->curNode->node->xyz;
			//check if line between curnode and prev node intersects with every line formed by 2 of snake1's nodes
			xyz_f snake2_p1 = snake2->prevNode->led->xyz;
			xyz_f snake2_p2 = snake2->curNode->led->xyz;
			//First let's get snake2's 2D coordinates
			xy_f snake2_xy1 = projectPoint(snake2_node_xyz, snake2_p1);
			xy_f snake2_xy2 = projectPoint(snake2_node_xyz, snake2_p2);
			for (int i = 1; i < snake1->pastNodes.size(); i++) {
				if (snake1->pastNodes[i]->nodeImpact() && snake1->pastNodes[i - 1]->nodeImpact()) {
					//Then let's get snake1's 2D coordinates
					xyz_f snake1_p1 = snake1->pastNodes[i]->led->xyz;
					xyz_f snake1_p2 = snake1->pastNodes[i - 1]->led->xyz;
					if (distance(snake2_p1, snake1_p1) > 100) {
						//the nodes are too far apart
						continue;
					}
					xy_f snake1_xy1 = projectPoint(snake2_node_xyz, snake1_p1);
					xy_f snake1_xy2 = projectPoint(snake2_node_xyz, snake1_p2);
					//Then run the line intersection between them
					if (linesIntersect(snake2_xy1, snake2_xy2, snake1_xy1, snake1_xy2)) {
						shouldKill2 = 1;
					}
				}
			}
	...
	if (xbox::getBHeld(P1) || xbox::getBHeld(P2)) {
		for (int i = 0; i < 4.0 * (xbox::getRightThumbY(P1) + 1) + 1; i++) {
			Raindrop* rain = new Raindrop(ledInterface, shared->mapping);
			if (xbox::getBHeld(P2)) {
				rain->on = { 0,1,0 };
			}
			effectEngine->apply(rain);
		}
		snake1->hue = 0;
		otherStuff = 1;
	}
